...
hg_base::load_controller('common', 'admin', 0);
...
class category extends common {
	...
	public function init() {
		...
		$tree = hg_base::load_sys_class('tree');
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ','&nbsp;&nbsp;&nbsp;├─ ','&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		...
		foreach($data as $v) {
			...
			$v['string'] = '<a title="增加子类" href="javascript:;" onclick="hg_open(\'增加栏目\',\''.U('add',array('modelid'=>$v['modelid'],'type'=>$v['type'],'catid'=>$v['id'])).'\',800,500)" class="btn-mini btn-secondary ml-5" style="text-decoration:none">增加子类</a>
			<a title="编辑栏目" href="javascript:;" onclick="hg_open(\'编辑栏目\',\''.U('edit',array('type'=>$v['type'],'catid'=>$v['id'])).'\',800,500)" class="btn-mini btn-success ml-5" style="text-decoration:none">编辑</a>
			<a title="删除" href="javascript:;" onclick="hg_del(\''.U('delete',array('type'=>$v['type'],'catid'=>$v['id'])).'\')" class="btn-mini btn-warning ml-5" style="text-decoration:none">删除</a>';
	...
	public function edit() {				
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['arrparentid']!=$_POST['cpath']){
				...
				$this->db->query("UPDATE hkingcms_category SET arrparentid=REPLACE(arrparentid, '{$_POST['cpath']}','{$_POST['arrparentid']}') WHERE arrparentid like '{$cpath}%'");
			}
	...
	*/
	...
	private function select_template($style, $pre = '') {
			$_tplpath=HKINGPHP_PATH . 'templates' . DIRECTORY_SEPARATOR . C('site_theme') . DIRECTORY_SEPARATOR . 'index' . DIRECTORY_SEPARATOR;
//			$_tplpath=APP_PATH.'index'.DIRECTORY_SEPARATOR.'view'.DIRECTORY_SEPARATOR.C('site_theme').DIRECTORY_SEPARATOR;
		    $files = glob($_tplpath.$pre.'*.html');
