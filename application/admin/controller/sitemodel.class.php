...
hg_base::load_common('lib/sql'.EXT, 'admin');
...
class sitemodel extends common {
	...
	public function delete() {
		...
		if(in_array($modelid, array(1))) showmsg('不能删除系统模型！');
		$model = D('model');		
	...
	public function add() {		
		...
		if(isset($_POST['dosubmit'])) {
			...
			$_POST['modelid'] = intval($model->total())+1;

			$model->insert($_POST, true,false);
	...
	public function edit() {				
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($model->update(array('name'=>$_POST["name"],'description'=>$_POST["description"],'disabled'=>$_POST["disabled"], 'issystem' => $_POST["issystem"]), array('modelid' => $modelid))){
				delcache('modelinfo');
				return_json(array('status'=>1,'message'=>L('operation_success')));
			}else{
				return_json();
	...
	public function export() {
		...
		header('Content-Disposition: attachment; filename="hg_'.$data['tablename'].'.model"');
		echo $res;exit;
	}
