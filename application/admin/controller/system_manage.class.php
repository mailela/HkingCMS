...
class system_manage extends common {
	...
	public function save() {
		hg_base::load_common('function/function.php', 'admin');
	...
	public function public_test_mail() {
		if(sendmail($_POST['mail_to'], 'HkingCMS邮件测试', '这是一封测试邮件，如果您成功接收此邮件，说明您的邮件配置正确！')){
			exit('发送邮件成功！');
		}else{
			exit('发送邮件失败！');
		}	
	...
	public function user_config_export() {
		...
		header('Content-Disposition: attachment; filename="hg_config.cfg"');
		echo $res;exit;
	}
