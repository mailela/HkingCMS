...
hg_base::load_controller('wechat_common', 'wechat', 0);
...
class material extends wechat_common{
	...
	public function add(){
		if(isset($_POST['dosubmit'])) {
			...
			//上传公众号一定要使用绝对路径
			$filepath = str_replace('\\', '/', rtrim(HKINGPHP_PATH, DIRECTORY_SEPARATOR)).$fileinfo['filepath'].$fileinfo['filename'];
	...
	public function delete(){ 
		...
		$file = HKINGPHP_PATH.strstr($info['filepath'].$info['filename'], C('upload_file'));
