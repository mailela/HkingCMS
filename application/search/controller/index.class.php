...
class index{
	...
	public function tag(){	
		...
		$page = new page($total, 5);
		$search_data = $tag_content->field('title,description,inputtime,updatetime,click,thumb,nickname,url')->join('`hkingcms_article` b ON hkingcms_tag_content.aid=b.id')->where(array('modelid'=>$modelid,'tagid'=>$id))->order('updatetime DESC')->limit($page->limit())->select();
