...
define("VERSION", 'HkingCMS 5.1');

if(is_file(SITEDIR.'cache'.DIRECTORY_SEPARATOR.'install.lock')){
    exit("HkingCMS程序已运行安装，如果你确定要重新安装，请先从FTP中删除 cache/install.lock！");
}
...
switch ($step) {
    ...
    case '4':
        if (isset($_GET['install'])) {
            ...
            $dbPrefix = empty($_POST['dbprefix']) ? 'hg_' : str_replace(array('"','\'',','), '', trim($_POST['dbprefix']));
...
function sql_split($sql, $tablepre) {
    ...
    if ($tablepre != "hg_")
        $sql = str_replace("hg_", $tablepre, $sql);
