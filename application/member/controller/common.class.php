...
class common{
	...
	public function get_message(){ 
		...
		$data = $this->db->fetch_array($this->db->query("SELECT COUNT(*) AS total FROM hkingcms_message_group a LEFT JOIN hkingcms_message_data b ON a.id=b.group_message_id WHERE a.groupid='{$this->memberinfo['groupid']}' AND a.`status`=1 AND b.userid={$this->memberinfo['userid']}"));  //已读信息
