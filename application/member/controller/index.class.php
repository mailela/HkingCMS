...
hg_base::load_controller('common', 'member', 0);
...
class index extends common{
	...
	public function user_pic(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$upload = hg_base::load_sys_class('upload');
			if($upload->uploadfile('user_pic')){
				...
				$userpic = HKINGPHP_PATH.ltrim($userpic, SITE_PATH);
	...
	public function follow_dynamic(){
		...
		hg_base::load_sys_class('page','',0);
		$total = $member_content->join('hkingcms_member_follow ON hkingcms_member_follow.followid = hkingcms_member_content.userid', 'RIGHT')->where("hkingcms_member_follow.userid=$userid AND status=1")->total();
		$page = new page($total, 10);
		$res = $member_content->field('hkingcms_member_content.checkid,hkingcms_member_content.catid,hkingcms_member_content.username,hkingcms_member_content.title,hkingcms_member_content.inputtime')->join('hkingcms_member_follow ON hkingcms_member_follow.followid = hkingcms_member_content.userid', 'RIGHT')->where("hkingcms_member_follow.userid=$userid AND status=1")->order('inputtime DESC')->limit($page->limit())->select();
		$data = array();
		foreach($res as $val) {
	...
	public function fans(){
		...
		$page = new page($total, 9);
		$data = $member_follow->alias('f')->field('m.userid,m.username')->join('hkingcms_member m ON f.userid=m.userid')->where(array('followid'=>$userid))->order('id DESC')->limit($page->limit())->select();
		$pages = '<span class="pageinfo">共'.$total.'条记录</span>'.$page->getfull();
