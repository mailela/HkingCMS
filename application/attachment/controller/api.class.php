...
hg_base::load_sys_class('upload','',0);
...
class api{
	...
	public function img_cropper(){
		$hkingcms_path = SITE_PATH == '/' ? HKINGPHP_PATH : str_replace(SITE_PATH, '', str_replace('\\', '/', HKINGPHP_PATH));
		if(isset($_POST['filepath'])){
			$x = $_POST['x'];
			$y = $_POST['y'];
			$w = $_POST['w'];
			$h = $_POST['h'];
			$image = hg_base::load_sys_class('image');
			$filepath = SITE_PATH.C('upload_file').'/'.date('Ym/d/');
			$filename = date("ymdhis").rand(100,999);
			$filetype = fileext($_POST['filepath']);
			$fileinfo = $image->crop($hkingcms_path.$_POST['filepath'], $hkingcms_path.$filepath.$filename.'.'.$filetype, $w, $h, $x, $y);
			if($fileinfo){
		...
		if(strpos($filepath, 'ttp:') || !is_file($hkingcms_path.$filepath)) showmsg('请选择本地已存在的图像！', 'stop');
