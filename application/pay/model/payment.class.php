...
class payment
{
    ...
    {
        hg_base::load_common('plugin/alipay/pagepay.php');
        $params = array(
            'subject' => $params['desc'],
    ...
    public function alipay_notify($params, $order)
    {
        ...
        hg_base::load_common('plugin/alipay/notify.php');
     ...
     * @param string $name
     */
    public static function wxpay_config()
    {
        if (!$wxpay = getcache('wechat')) {
            $data = D('pay_mode')->where(array('action' => 'wechat'))->find();
            if (!$data) showmsg('支付方式不存在！', 'stop');
            $wxpay = string2array($data['config']);
            $wxpay['notify_url'] = U('pay/index/pay_callback');  //异步通知地址
            $wxpay['return_url'] = U('pay/index/pay_return');    //同步跳转
            $wxpay['charset'] = 'UTF-8';  //编码格式
            $wxpay['sign_type'] = 'RSA2';  //签名方式
            setcache('wxpay', $wxpay);
        }
        return $wxpay;
    }
    ...
    public function update_order($order, $transaction)
    {
        ...
        if ($result) {
            // 充值积分
            $point = hg_base::load_model('point', 'member');
    ...
    {
        hg_base::load_common('plugin/wxpay/native.php');  //如需微信支付模块联系QQ 214243830
        $params = array(
            'body' => $params['desc'],
            'out_trade_no' => $params['order_sn'],
            'total_fee' => $params['money'] * 100,  //微信支付单位为 ：分
            'money' => $params['money'] * 100,  //微信支付单位为 ：分
            'title' => $params['desc'],  //微信支付单位为 ：分
