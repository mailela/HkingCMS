...
defined('HKINGPHP_PATH') or exit('Access Denied.');

define('IN_HKINGPHP', true);

//HKINGPHP框架路径
define('YP_PATH', dirname(__FILE__).DIRECTORY_SEPARATOR);
//HKINGPHP框架版本信息
define('HKINGPHP_VERSION', '2.0');
//HKINGPHP应用目录
define('APP_PATH', HKINGPHP_PATH.'application'.DIRECTORY_SEPARATOR);
...
//加载公用函数库
hg_base::load_sys_func('global');
hg_base::load_common('function/system.func.php');
hg_base::load_common('function/extention.func.php');
hg_base::load_common('data/version.php');
defined('HkingCMS_SOFTNAME') or define('HkingCMS_SOFTNAME', base64_decode('SGtpbmdDTVM='));
...
class hg_base {
	...
	private static function _load_class($classname, $path = '', $initialize = 1) {
		...
		if (empty($path)) $path = HKINGPHP_PATH.'hkingphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'class';
...
//		die($path.DIRECTORY_SEPARATOR.$classname.EXT);
	...
	public static function load_sys_func($func) {
		if (is_file(HKINGPHP_PATH.'hkingphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php')) {
			include HKINGPHP_PATH.'hkingphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php';
		}
	}
	...
	public static function load_common($path, $m = '') {
		if(!$m){
			if (is_file(HKINGPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path)) {
				return include HKINGPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path;
			}else{
				debug::addmsg(HKINGPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path.L('does_not_exist'));
			}			
		}else{
