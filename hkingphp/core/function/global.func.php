 ...
 * @return array
 */
function string2array2($val)
{
    $val = rtrim($val, ',');
    $_data = explode(",", trim($val));
//    if(!empty($_data)&& !empty($val)){
//        print($val);
//        die();
//    }
    $data = array();
 ...
 * @return bool
 */
function inarray($search_key, $array)
{
...
function thumb($imgurl, $width = 300, $height = 200, $autocut = 0, $smallpic = 'nopic.jpg')
{
    ...
    $upload_path = HKINGPHP_PATH . C('upload_file') . '/';
    ...
    if (!is_object($image)) {
        hg_base::load_sys_class('image', '', '0');
        $image = new image(1);
    }
...
function set_cookie($name, $value = '', $time = 0)
{
    ...
    $value = is_array($value) ? 'in_hgphp' . string_auth(json_encode($value)) : string_auth($value);
...
function get_cookie($name = '', $default = '')
{
    ...
    if (isset($_COOKIE[$name])) {
        if (strpos($_COOKIE[$name], 'in_hgphp') === 0) {
...
{
    return hg_base::load_model($classname);
}
...
function D($tabname)
{
    ...
    hg_base::load_sys_class('db_factory', '', 0);
...
function C($key = '', $default = '')
{
    ...
    $path = HKINGPHP_PATH . 'common' . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'config.php';
    if (is_file($path)) {
...
{
    hg_base::load_sys_class('cache_factory', '', 0);
...
function template($module = '', $template = 'index')
{
    ...
    $template_c = HKINGPHP_PATH . 'cache' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR;

    if (ISROOT_TEMPLATE) {
        $template_path = !defined('MODULE_THEME') ? HKINGPHP_PATH . 'templates' . DIRECTORY_SEPARATOR . C('site_theme') . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR : APP_PATH . $module . DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . MODULE_THEME . DIRECTORY_SEPARATOR;;
        if (!defined("TPL")) {
            define('TPL', '/templates/' . C('site_theme'));
        }  //模板路径
    } else {
        ...
        if (!defined("TPL")) {
            define('TPL', str_replace(APP_PATH, "/application/", $template_path));
        }  //模板路径
    }
    ...
    if (empty($template)) {
        $tplfile = rtrim($template_path,"/").'.html';
    } else {
    ...
    if (!is_file($tplfile)) {
        showmsg(str_replace(HKINGPHP_PATH, '', $tplfile) . L('template_does_not_exist'), 'stop');
    }
    if (!is_dir(HKINGPHP_PATH . 'cache' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR)) {
        @mkdir(HKINGPHP_PATH . 'cache' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR, 0777, true);
    }
    $template_c = $template_c . $template . '.tpl.php';
    if (!is_file($template_c) || filemtime($template_c) < filemtime($tplfile)) {
        $hg_tpl = hg_base::load_sys_class('hg_tpl');
        $compile = $hg_tpl->tpl_replace(@file_get_contents($tplfile));
...
function write_log($message, $filename = '', $path = '')
{
    ...
    if (!$path) $path = HKINGPHP_PATH . 'cache';
