...
class hg_tag{
	...
	public function centent_tag($data) {
		...
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return  D('tag_content')->field('id,tag')->join('hkingcms_tag b ON hkingcms_tag_content.tagid=b.id')->order('id DESC')->where('hkingcms_tag_content.modelid='.$modelid.' AND hkingcms_tag_content.aid='.$id)->limit($limit)->select();
	}	
	...
	public function comment_newest($data) {
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return D('comment')->field('userid,username,userpic,inputtime,content,title,url,catid,modelid')->join('hkingcms_comment_data b ON hkingcms_comment.commentid=b.commentid')->where('`status`=1')->order('id DESC')->limit($limit)->select();
	}
