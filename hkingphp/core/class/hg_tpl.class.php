...
class hg_tpl {
	...
	public function tpl_replace($str) {
		...
		$str = preg_replace_callback("/".$this->template_tag_left."m:(\w+)\s+([^}]+)".$this->template_tag_right."/i", array($this, 'hg_tag_callback'), $str);

		$str = "<?php defined('IN_HKINGPHP') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}	

	
	public static function hg_tag_callback($matches) {
		return self::hg_tag($matches[1],$matches[2], $matches[0]);;
	}
	 ...
	 * 解析标签
	 */	
	public static function hg_tag($action, $data, $html) {
		...
		$str = '$tag = hg_base::load_sys_class(\'hg_tag\');';
