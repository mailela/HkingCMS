...
class image {
    ...
    public function __construct($thumb_enable = 0 , $watermark_enable = 0) {
		...
		$this->w_img = HKINGPHP_PATH.'common/data/water/'.C('watermark_name');
    }
	 ...
	 */
	public function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'hkingcms', $w_font = 8, $w_color = '#ff0000') {
		$w_pos = $w_pos ? $w_pos : $this->w_pos;
		$w_img = $w_img ? HKINGPHP_PATH.'common/data/water/'.$w_img : $this->w_img;
		...
		if(!empty($w_img) && is_file($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, HKINGPHP_PATH.'common/data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
