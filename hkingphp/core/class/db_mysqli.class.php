...
class db_mysqli{
	 ...
	 * @param $linknum 	数据库编号	
	 * @param $config 	array	
	 * @参数说明		array('db_host'=>'127.0.0.1', 'db_user'=>'root', 'db_pwd'=>'', 'db_name'=>'hkingcms', 'db_port'=>3306, 'db_prefix'=>'hg_')
	 *					[服务器地址, 数据库用户名, 数据库密码, 数据库名, 服务器端口, 数据表前缀]
	 * 						
	 * 使用方法(添加一个编号为1的数据库连接，并自动切换到当前的数据库连接):  
	 * D('tablename')->db(1, array('db_host'=>'127.0.0.1', 'db_user'=>'root', 'db_pwd'=>'', 'db_name'=>'test', 'db_port'=>3306, 'db_prefix'=>'hg_'))->select();
	 * 
	...
	public function where($arr = ''){
		...
		if(is_array($arr)) {
			...
			$this->key['where'] = str_replace('hkingcms_', $this->config['db_prefix'], $arr);
			return $this;
		}
	}
	 ...
	 *                      结果为： DELETE FROM `hkingcms_admin` WHERE id IN (3,4,5);
	 *
	 * @return int          返回影响行数
	 */
	...
	public function select(){
        $rs = array();		
		$field = isset($this->key['field']) ? str_replace('hkingcms_', $this->config['db_prefix'], $this->key['field']) : ' * ';
	...
	/**
	 * 链接查询
	 * @param $join 	string SQL语句，如hkingcms_admin ON hkingcms_admintype.id=hkingcms_admin.id
	 * @param $type 	可选参数,默认是inner
	 * @return object
	 */	
	public function join($join, $type = 'INNER'){
		$join = str_replace('hkingcms_', $this->config['db_prefix'], $join);
	...
	public function query($sql = ''){
		 $sql = str_replace('hkingcms_', $this->config['db_prefix'], $sql);
