...
{
    $theme_list = array();
    $theme_path = HKINGPHP_PATH . 'templates'  . DIRECTORY_SEPARATOR. '*';
    if ($m != "index") {
        $theme_path = APP_PATH . $m . DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . '*';
    }

    $list = glob($theme_path, GLOB_ONLYDIR);
//    var_dump($theme_path);
//    die();
    foreach ($list as $v) {
...
function get_tags($tags = '')
{
    ...
    $string .= '<a href="javascript:;" onclick="hg_open(\'添加TAG\',\'' . U('admin/tag/add') . '\',\'600\',\'300\')" class="hkingcms_a">点击添加</a>';
    return $string;
}
...
{
    if (!$arr) exit(json_encode(array('status' => 0, 'message' => L('data_not_modified'),"post"=>$_POST)));
...
function get_memberavatar($user, $type = 1, $default = true)
{
    ...
    if ($type == 1) {
        ...
        $data = $member_detail->field('userpic')->join('hkingcms_member b ON hkingcms_member_detail.userid=b.userid')->where(array('username' => $user))->find();
    }
